...
For each model $j$ the mean squared error of the model parameters $MSE_l(\tilde{\beta}_k,\hat{\beta})=\frac{1}{MJ} \sum_{i=1}^M \sum_{j=1}^J (\tilde{\beta}_{k,j} - \hat{\beta}_j)^2$ are calculated for the $M=250$ simulations across the sample sizes $k=(800,\ldots,2000)$ and the initial sample size is $r0=400$. 
...
NeEDS4BigData::modelRobustLogSub(r0=400,r=rep_k,
...
for (i in 1:length(All_Models)) {
  ...
  MSE_Beta_MR<-data.frame("Method"=Final_Beta_modelRobust[[i]]$Method,
                          "Sample"=Final_Beta_modelRobust[[i]]$r,
                          "SE"=rowSums((All_Beta - 
...
NeEDS4BigData::modelMissLogSub(r0=400,r=rep_k,
...
Mean_AMSE<-Final_AMSE_modelMiss |> dplyr::group_by(r,Method) |> 
       dplyr::summarise(Mean=mean(AMSE),
                        min=quantile(AMSE,0.05),
                        max=quantile(AMSE,0.95),.groups ='drop')

ggplot(Mean_AMSE,aes(x=factor(r),y=Mean,color=Method,shape=Method)) +
  geom_point(size=3,position = position_dodge(width = 0.5))+
  geom_line(data=Mean_AMSE,aes(x=factor(r),y=Mean,group=Method,color=Method),
            position = position_dodge(width = 0.5))+
