...
This data contains $7$ columns and has $205,032$ observations, first column is the response variable and the rest are covariates. 
We consider the covariates a) duration ($x_1$), b) loudness ($x_2$), c) tempo ($x_3$), d) artist hotness ($x_4$), e) song hotness ($x_5$), and f) album hotness ($x_6$) to model the response, the number of play counts for each song.
...
Here five models are selected out of eight models, which are 1) main effects model ($\beta_0+\beta_1X_1+\beta_2X_2+\beta_3X_3+\beta_4X_4+\beta_5X_5+\beta_6X_6$), 2-7) main effects model with the squared term of each covariate ($X^2_1 / X^2_2 / X^2_3 / X^2_4 / X^2_5 / X^2_6$), 8-22) main effects model with a pair of squared terms, 23-42) main effects model with three of a kind squared terms, 43-57) main effects model with four of a kind squared terms, 58-63) main effects model with five of a kind squared terms and 64) main effects model with all squared terms. 
The five models are selected based on the smallest AIC values.  
For each model $j$ the mean squared error of the model parameters $MSE_l(\tilde{\beta}_k,\hat{\beta})=\frac{1}{MJ} \sum_{i=1}^M \sum_{j=1}^J (\tilde{\beta}_{k,j} - \hat{\beta}_j)^2$ are calculated for the $M=250$ simulations across the sample sizes $k=(800,\ldots,2000)$ and the initial sample size is $r1=400$. 
...
NeEDS4BigData::modelRobustPoiSub(r1=400,r2=rep_k,
...
NeEDS4BigData::modelMissPoiSub(r1=400,r2=rep_k,
