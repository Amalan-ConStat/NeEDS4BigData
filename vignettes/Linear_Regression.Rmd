...
For each model $l$ the mean squared error of the model parameters $MSE_l(\tilde{\beta}_k,\hat{\beta})=\frac{1}{MJ} \sum_{i=1}^M \sum_{j=1}^J (\tilde{\beta}_{k,j} - \hat{\beta}_j)^2$ are calculated for the $M=100$ simulations across the sample sizes $k=(600,\ldots,1500)$ and the initial sample size is $r1=300$. 
...
The final scenario is for comparison of the sampling method under the assumption that the main effects model is potentially misspecified against the $A$- and $L$-optimality subsampling method. 
...
```{r Potential model misspecification methods,fig.width=6,fig.height=6,fig.align='center', fig.cap="AMSE for the potentially misspecified main effects model across the sampling methods under comparison."}
# Define the sampling methods and their respective colours, shapes and line types
