...
modelMissPoiSub <- function(r1,r2,Y,X,N,Alpha,proportion,model="Auto"){
  ...
  L_All<-foreach::foreach(a=1:N,.combine = rbind,.packages = "psych") %dopar% {
    ...
    lambda_r1<-c(lambda.prop,Lambda.prop[a])
    W_r1<-c(w.prop,W_All[a])

    lambda_Tr1<-c(lambda_Tr.prop,lambda_Tr[a])
  ...
  for (i in 1:length(r2))
  {
    ...
    beta_L1_optimality[i,] <- c(r2[i],fit.L1_optimality$coefficients)
    ...
    b_r1 <- crossprod(x.L1_optimality, lambda_Tr1-lambda_r1)
