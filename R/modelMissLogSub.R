#' Model misspecified subsampling under logistic regression
#'
#' Using this function subsample from big data under logistic regression when we assumed the model that
...
modelMissLogSub <- function(r1,r2,Y,X,N,Alpha,Beta_Estimate_Full,F_Estimate_Full){
  ...
  for (i in 1:length(r2))
  {
    ...
    fit.LmAMSE <- .getMLE(x=x.LmAMSE, y=y.LmAMSE,w=c(1 / PI.LmAMSE[idx.LmAMSE]))

    beta_LmAMSE[i,] <- c(r2[i],fit.LmAMSE$par)

    idx.LmAMSE->Sample.LmAMSE[[i+1]]

    p_r1<-1-1/(1+exp(x.LmAMSE%*%Beta_Estimate_Full))
    W_r1<-as.vector(p_r1*(1-p_r1))
    H_r1 <-solve(t(x.LmAMSE) %*% (x.LmAMSE * W_r1))
    Temp1<-(W_r1 * x.LmAMSE)%*%H_r1

    p_Tr1<-1-1/(1+exp((x.LmAMSE %*% Beta_Estimate_Full) + F_Estimate_Full[c(idx.LmAMSE)]))
    W_Tr1<-as.vector(p_Tr1*(1-p_Tr1))
    H_Tr1 <-(t(x.LmAMSE) %*% (x.LmAMSE * W_Tr1))
    b_r1 <-(t(x.LmAMSE) %*% (p_Tr1-p_r1))
    L1_r1 <- psych::tr(Temp1%*%H_Tr1%*%t(Temp1))
    ...
    for (j in 1:length(Alpha))
    {
      ...
      fit.LmAMSE <- .getMLE(x=x.LmAMSE, y=y.LmAMSE,w=c(1 / PI.LmAMSE_LO[idx.LmAMSE,j]))

      beta_LmAMSE_LO[[j]][i,] <- c(r2[i],fit.LmAMSE$par)
      ...
      fit.LmAMSE <- .getMLE(x=x.LmAMSE, y=y.LmAMSE,w=c(1 / PI.LmAMSE_Pow[idx.LmAMSE,j]))

      beta_LmAMSE_Pow[[j]][i,] <- c(r2[i],fit.LmAMSE$par)
