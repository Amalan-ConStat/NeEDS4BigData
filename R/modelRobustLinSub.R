...
#' @export
modelRobustLinSub <- function(r0,rf,Y,X,N,Apriori_probs,All_Combinations,All_Covariates){
  ...
  Var_Epsilon_mVc<-Var_Epsilon_mMSE<-matrix(nrow = length(rf),ncol = length(All_Combinations) + 1)
  Sample.mMSE_Single<-Sample.mVc_Single<-list()

  # Model Robust Results
  beta.mVc_MR<-beta.mMSE_MR<-list()
  Var_Epsilon_mVc_MR<-Var_Epsilon_mMSE_MR<-matrix(nrow = length(rf),ncol = length(All_Combinations) + 1)
  Sample.mMSE_MR<-Sample.mVc_MR<-list()

  Var_Epsilon_mVc[,1]<-Var_Epsilon_mMSE[,1]<-Var_Epsilon_mVc_MR[,1]<-Var_Epsilon_mMSE_MR[,1]<-rf
  ...
  for (i in 1:length(rf))
  {
    ...
    fit_Single.mVc <-lapply(1:length(All_Combinations), function(j){ # Single Model Results
      ...
      pi4_r<-sqrt(rf[i]*pinv_Single.mVc^(-1))
    ...
    for (j in 1:length(All_Combinations))
    {
      ...
      beta.mVc_Single[[j]][i,] <- c(rf[i],fit_Single.mVc[[j]]$beta.prop)
      beta.mVc_MR[[j]][i,] <- c(rf[i],fit_MR.mVc[[j]]$beta.prop)
    ...
    idx_Single.mMSE <- lapply(1:length(All_Combinations),function(j){
      sample(1:N, size = rf[i]-r0, replace = T, prob = PI_Single.mMSE[[j]]) # Single Model Results
    })
    idx_MR.mMSE <- sample(1:N, size = rf[i]-r0, replace = T, prob = PI_MR.mMSE) # Model Robust Results
    ...
    fit_Single.mMSE <-lapply(1:length(All_Combinations), function(j){ # Single Model Results
      ...
      pi4_r<-sqrt(rf[i]*pinv_Single.mMSE^(-1))
    ...
    fit_MR.mMSE <- lapply(1:length(All_Combinations),function(j){
      pi4_r<-sqrt(rf[i]*pinv_MR.mMSE^(-1))
    ...
    for (j in 1:length(All_Combinations))
    {
      ...
      beta.mMSE_Single[[j]][i,] <- c(rf[i],fit_Single.mMSE[[j]]$beta.prop)
      beta.mMSE_MR[[j]][i,] <- c(rf[i],fit_MR.mMSE[[j]]$beta.prop)
  ...
  colnames(Var_Epsilon_Data)[-1]<-c("rf",paste0("Model_",1:length(All_Combinations)))
