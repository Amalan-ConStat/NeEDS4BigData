...
test_that("dimension of the LCC sample",{
  expect_equal(length(Results$Sample_LCC_Sampling),c(length(rf)+1))
})

context_start_file("Checking the LCCsampling for error messages")
test_that("Error on input for r",{
  expect_error(LCCsampling(r0 = r0, rf = c(rf,NA), Y = as.matrix(Original_Data[,colnames(Original_Data) %in% c("Y")]),
                           X = as.matrix(Original_Data[,-1]),
                           N = nrow(Original_Data)),"NA or Infinite or NAN values in the r0,rf or N")
})
Original_Data[100,]<-rep(NA,4)
test_that("Error on X input",{
  expect_error(LCCsampling(r0 = r0, rf = rf, Y = as.matrix(Original_Data[,colnames(Original_Data) %in% c("Y")]),
                           X = as.matrix(Original_Data[,-1]),
...
test_that("Error on r0 and r input",{
  expect_error(LCCsampling(r0 = r0+1000, rf = rf, Y = as.matrix(Original_Data[,colnames(Original_Data) %in% c("Y")]),
                           X = as.matrix(Original_Data[,-1]),
