...
AoptimalMCGLMSub(r0 = r0, r = r,
                 Y = as.matrix(Original_Data[,colnames(Original_Data) %in% c("Y")]),
                 X = as.matrix(Original_Data[,-1]),N = nrow(Original_Data),
                 family = "linear") |> suppressMessages()->Results

context_start_file("Checking the AoptimalMCGLMSub function under linear regression")
...
context_start_file("Checking the AoptimalMCGLMSub function under logistic regression")
test_that("type of the Results",{
  expect_type(Results,"list")
})
...
context_start_file("Checking the AoptimalMCGLMSub for error messages")
test_that("Error on input for r",{
  expect_error(AoptimalMCGLMSub(r0 = r0, r = c(r,NA),
                                Y = as.matrix(Original_Data[,colnames(Original_Data) %in% c("Y")]),
                                X = as.matrix(Original_Data[,-1]),N = nrow(Original_Data),
                                family = "logistic"),"NA or Infinite or NAN values in the r0,r,N or family")
})
Original_Data[100,]<-rep(NA,4)
test_that("Error on X input",{
  expect_error(AoptimalMCGLMSub(r0 = r0, r = r,
                                Y = as.matrix(Original_Data[,colnames(Original_Data) %in% c("Y")]),
                                X = as.matrix(Original_Data[,-1]),N = nrow(Original_Data),
...
test_that("Error on r0 and r input",{
  expect_error(AoptimalMCGLMSub(r0 = r0+1000, r = r,
                                Y = as.matrix(Original_Data[,colnames(Original_Data) %in% c("Y")]),
                                X = as.matrix(Original_Data[,-1]),N = nrow(Original_Data),
                                family = "logistic"),"2*r0 cannot be greater than r at any point")
})
...
context_start_file("Checking the AoptimalMCGLMSub function under poisson regression")
test_that("type of the Results",{
  expect_type(Results,"list")
})
...
test_that("Error on input for r",{
  expect_error(AoptimalMCGLMSub(r0 = r0, r = c(r,NA),
                                Y = as.matrix(Original_Data[,colnames(Original_Data) %in% c("Y")]),
                                X = as.matrix(Original_Data[,-1]),N = nrow(Original_Data),
                                family = "poisson"),"NA or Infinite or NAN values in the r0,r,N or family")
})
Original_Data[100,]<-rep(NA,4)
test_that("Error on X input",{
  expect_error(AoptimalMCGLMSub(r0 = r0, r = r,
...
test_that("Error on r0 and r input",{
  expect_error(AoptimalMCGLMSub(r0 = r0+1000, r = r,
                                Y = as.matrix(Original_Data[,colnames(Original_Data) %in% c("Y")]),
                                X = as.matrix(Original_Data[,-1]),N = nrow(Original_Data),
                                family = "poisson"),"2*r0 cannot be greater than r at any point")
})
test_that("Error on size of X and Y",{
  expect_error(AoptimalMCGLMSub(r0 = r0, r = r,
                                Y = as.matrix(Original_Data[,colnames(Original_Data) %in% c("Y")]),
