...
Results<-modelMissPoiSub(r0 = r0, r = r,Y = as.matrix(Original_Data[,1]),
...
test_that("dimension of the Beta Estimates",{
  expect_equal(dim(Results$Beta_Estimates),c(length(r)*6,length(Beta)-1+2))
})
...
test_that("dimension of the L-optimality sample",{
  expect_equal(length(Results$`Sample_L-Optimality`),c(length(r)+1))
})

test_that("dimension of the L1-optimality sample",{
  expect_equal(length(Results$`Sample_L1-Optimality`),c(length(r)+1))
})

test_that("dimension of the RLmAMSE sample",{
  expect_equal(length(Results$Sample_RLmAMSE),c(length(r)+1))
})

test_that("dimension of the RLmAMSE Log Odds sample",{
  expect_equal(length(Results$Sample_RLmAMSE_Log_Odds[[1]]),c(length(r)+1))
})

test_that("dimension of the RLmAMSE Power sample",{
  expect_equal(length(Results$Sample_RLmAMSE_Power[[1]]),c(length(r)+1))
})

context_start_file("Checking the modelMissPoiSub for error messages")
test_that("Error on input for r",{
  expect_error(modelMissPoiSub(r0 = r0, r = c(r,NA),Y = as.matrix(Original_Data[,1]),
                               X = as.matrix(Original_Data[,-1]),
                               N = N,Alpha = 10, proportion = 0.3),
               "NA or Infinite or NAN values in the r0,r,N,Alpha or proportion")
})
Original_Data[100,]<-rep(NA,4)
test_that("Error on X input",{
  expect_error(modelMissPoiSub(r0 = r0, r = r,Y = as.matrix(Original_Data[,1]),
                               X = as.matrix(Original_Data[,-1]),
                               N = N,Alpha = 10, proportion = 0.3),
...
test_that("Error on r0 and r input",{
  expect_error(modelMissPoiSub(r0 = r0+1000, r = r,Y = as.matrix(Original_Data[,1]),
                               X = as.matrix(Original_Data[,-1]),
                               N = N,Alpha = 10, proportion = 0.3),
...
test_that("Error on proportion value",{
  expect_error(modelMissPoiSub(r0,r,Y = as.matrix(Original_Data[,1]),
                               X = as.matrix(Original_Data[,-1]),
