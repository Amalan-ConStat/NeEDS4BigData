...
Results<-modelMissPoiSub(r1 = r1, r2 = r2,Y = as.matrix(Original_Data[,1]),
...
test_that("Error on input for r2",{
  expect_error(modelMissPoiSub(r1 = r1, r2 = c(r2,NA),Y = as.matrix(Original_Data[,1]),
                               X = as.matrix(Original_Data[,-1]),
                               N = Full_Data$N,Alpha = 10,
                               Beta_Estimate_Full = Full_Data$Beta$Estimate,
...
test_that("Error on r1 and r2 input",{
  expect_error(modelMissPoiSub(r1 = r1+1000, r2 = r2,Y = as.matrix(Original_Data[,1]),
                               X = as.matrix(Original_Data[,-1]),
                               N = Full_Data$N,Alpha = 10,
                               Beta_Estimate_Full = Full_Data$Beta$Estimate,
                               F_Estimate_Full = Full_Data$f$Real_GAM),
