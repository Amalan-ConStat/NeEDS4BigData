...
test_that("dimension of the Var Epsilon Estimates",{
  expect_equal(dim(Results$Variance_Epsilon_Estimates),c(length(rf),6+1))
})
...
test_that("Error on input for r",{
  expect_error(modelMissLinSub(r0,rf=NA,Y = as.matrix(Original_Data[,1]),
                               X = as.matrix(Original_Data[,-1]),
...
test_that("Error on X input",{
  expect_error(modelMissLinSub(r0,rf,Y = as.matrix(Original_Data[,1]),
                               X = as.matrix(Original_Data[,-1]),
                               N = N,Alpha = 10,proportion = 0.3),
...
test_that("Error on r0 and r input",{
  expect_error(modelMissLinSub(r0=r0+1000,rf,Y = as.matrix(Original_Data[,1]),
                               X = as.matrix(Original_Data[,-1]),
                               N = N,Alpha = 10, proportion = 0.3),
