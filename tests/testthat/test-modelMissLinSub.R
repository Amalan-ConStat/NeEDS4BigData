...
test_that("dimension of the Var Epsilon Estimates",{
  expect_equal(dim(Results$Variance_Epsilon_Estimates),c(length(r),6+1))
})
...
test_that("value in variance epsilon estimates",{
  expect_gt(min(Results$Variance_Epsilon_Estimates[,-1]),0)
})
...
test_that("Error on input for r",{
  expect_error(modelMissLinSub(r0,r=NA,Y = as.matrix(Original_Data[,1]),
                               X = as.matrix(Original_Data[,-1]),
...
test_that("Error on X input",{
  expect_error(modelMissLinSub(r0,r,Y = as.matrix(Original_Data[,1]),
                               X = as.matrix(Original_Data[,-1]),
                               N = N,Alpha = 10,proportion = 0.3),
...
test_that("Error on r0 and r input",{
  expect_error(modelMissLinSub(r0=r0+1000,r,Y = as.matrix(Original_Data[,1]),
                               X = as.matrix(Original_Data[,-1]),
                               N = N,Alpha = 10, proportion = 0.3),
