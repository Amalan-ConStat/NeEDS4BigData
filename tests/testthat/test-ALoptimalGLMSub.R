...
r0<-300; r<-rep(c(600,1200),10); Original_Data<-Full_Data$Complete_Data;

ALoptimalGLMSub(r0 = r0, r = r,
...
test_that("dimension of the Beta Estimates",{
  expect_equal(dim(Results$Beta_Estimates),c(length(r)*2,length(Beta)+2))
})
...
test_that("dimension of the Var Epsilon Estimates",{
  expect_equal(dim(Results$Variance_Epsilon_Estimates),c(length(r)*2,3))
})
...
test_that("Error on input for r",{
  expect_error(ALoptimalGLMSub(r0 = r0, r = c(r,NA),
                               Y = as.matrix(Original_Data[,colnames(Original_Data) %in% c("Y")]),
                               X = as.matrix(Original_Data[,-1]),N = nrow(Original_Data),
...
test_that("Error on X input",{
  expect_error(ALoptimalGLMSub(r0 = r0, r = r,
                               Y = as.matrix(Original_Data[,colnames(Original_Data) %in% c("Y")]),
                               X = as.matrix(Original_Data[,-1]),N = nrow(Original_Data),
...
test_that("Error on r0 and r input",{
  expect_error(ALoptimalGLMSub(r0 = r0+1000, r = r,
                               Y = as.matrix(Original_Data[,colnames(Original_Data) %in% c("Y")]),
                               X = as.matrix(Original_Data[,-1]),N = nrow(Original_Data),
